...
class ClickTestCase(TestBase, IsolatedAsyncioTestCase):
    ...
    @mock.patch(
        ...
        [
            ...
            "--token=s3cr3t",
            "--db-url",
            "postgres://user:hunter2@example.com:5432/db",
            "--opt",
            "argument",
        ],
    )
    def test_cli_command_wrapping_redacts_secrets(self):
        @asyncclick.command()
        @asyncclick.argument("argument")
        @asyncclick.option("--password")
        @asyncclick.option("--token")
        @asyncclick.option("--db-url")
        @asyncclick.option("--opt/--no-opt", default=False)
        async def command(
            argument: str, password: str, token: str, db_url: str, opt: bool
        ) -> None:
            pass
        ...
        self.assertEqual(
            ...
            (
                ...
                "--token=REDACTED",
                "--db-url",
                "postgres://REDACTED:REDACTED@example.com:5432/db",
                "--opt",
                "argument",
            ),
        )
    ...
    def test_cli_command_wrapping_redacts_aliases_and_keeps_flags(self):
        @asyncclick.command()
        @asyncclick.argument("argument")
        @asyncclick.option("-p", "--password")
        @asyncclick.option("--apikey")
        @asyncclick.option("--auth", is_flag=True)
        async def command(
            argument: str, password: str, apikey: str, auth: bool
        ) -> None:
            pass

        result = run_asyncclick_command_test(
            command,
            ["-p", "hunter2", "--apikey=s3cr3t", "--auth", "argument"],
        )
        self.assertEqual(result.exit_code, 0)
